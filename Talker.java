...
public class Talker implements Runnable{
	...
	private String processRequest(String request)
	{
		...
		if (request.startsWith("CLOSEST")) {
			...
			long id = Long.parseLong(request.split("_")[2]);
			result = local.find_successor(id);
			int value = Integer.parseInt(request.split("_")[1]);
			local.addValue(value);
			String ip = result.getAddress().toString();
			int port = result.getPort();
			ret = "Value added for : _"+ip+":"+port;
		}
		else if(request.startsWith("DELETE")) {
			long id = Long.parseLong(request.split("_")[2]);
			result = local.find_successor(id);
			int value = Integer.parseInt(request.split("_")[1]);
			local.deleteValue(value);
			String ip = result.getAddress().toString();
			int port = result.getPort();
			ret = "Value deleted for :_"+ip+":"+port;
		}
		else if(request.startsWith("UPDATE")) {
			long id = Long.parseLong(request.split("_")[3]);
			result = local.find_successor(id);
			int oldValue = Integer.parseInt(request.split("_")[1]);
			int newValue = Integer.parseInt(request.split("_")[2]);
			local.updateValue(oldValue,newValue);
			String ip = result.getAddress().toString();
			int port = result.getPort();
			ret = "Value updated for :_"+ip+":"+port;
		}
